...
describe('coordinator and visual safety',()=>{
 it('routes deterministic intents',()=>{expect(routeRequest('environmental risk')).toBe('terra-risk');expect(routeRequest('self-play benchmark')).toBe('cube-selfplay');expect(routeRequest('improve visual readability')).toBe('visual-readability')})
