...
const cubeRevision = 'e022239d15379127c4cda45e4b581b33ad48201c'
...
try {
  ...
  try {
    ...
    if (result?.directGuestActivated !== 'true') throw new Error(`AuthGate direct guest path did not complete: ${JSON.stringify(result)}`)
    if (result?.storedGuestMode !== 'guest') throw new Error(`Guest identity was not created by the normal guest contract: ${JSON.stringify(result)}`)
